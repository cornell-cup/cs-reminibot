...
int encode(char arr[], int len)
{
  ...
  for (int i = 0; i < len; i++)
  {
    crc = ((crc << 8) ^ table[((crc >> 8) ^ arr[i]) & 0xFF]) & 0xFFFF;
  }
  if (crc == 0)
  {
    crc = 1;
...
/**
 * Validate a message with start chars, end chars, and CRC16 sum
 *
 * Params:
 *  - arr[] is the message
 *  - len is the length of the message
 */
int validate_msg(char arr[], int len)
{
  // Message
  ...
  return start_ok && end_ok && hash_ok;
}
