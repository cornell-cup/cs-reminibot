  ...
  for (let i = 0; i < ports.length; i++) {
    let element = (
      <div key={i} className="form-group row">
        ...
        <button className="btn btn-secondary"             onClick={() => alert(selectedPorts[motorPorts[i]])} => handleButtonClick(event, i)}>{selectedValues[i] || 'Heldlo'}</button>
