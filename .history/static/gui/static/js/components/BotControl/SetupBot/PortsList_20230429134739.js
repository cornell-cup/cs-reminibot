...
function Ports(props) {
  ...
  const connectionNames = [
    "", "left", "right", "MOTOR3", "LINE", "INFRARED", "RFID", "ULTRASONIC"
  ]
...
export default function PortsList({ motorPorts, selectedBotName }) {
  ...
  const handleClickFunctions = [
    ...
    () => alert(`Button 2 clicked with value: ${selectedValues[1]}`),
  ...
  for (let i = 0; i < ports.length; i++) {
    let element = (
      <div key={i} className="form-group row">
        ...
        <button className="btn btn-secondary" onClick={handleClickFunctions[i]}> {(selectedValues[i]=='')?? 'Test'+selectedValues[i]'}</button>
