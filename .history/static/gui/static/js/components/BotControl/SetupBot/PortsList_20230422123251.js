import React, { useState } from 'react';

function Ports(props) {

  const connections = [
    { name: "", label: "Select connection..." },
    { name: "LMOTOR", label: "Left Motor" },
    { name: "RMOTOR", label: "Right Motor" },
    { name: "MOTOR3", label: "Motor 3" },
    { name: "LINE", label: "Line Follower" },
    { name: "INFRARED", label: "Infrared" },
    { name: "RFID", label: "RFID" },
    { name: "ULTRASONIC", label: "Ultrasonic" }
  ];

  let optionList = [];

  for (let i = 0; i < connections.length; i++) {
    optionList.push(
      <option key={i} value={() => alert(connections[i].name)}>
        {connections[i].label}
      </option>
    );
  }

  const handleSelectChange = (event) => {
    const func = event.target.value;
    func();
  }

  return (
    <select className="custom-select custom-select-sm" name={props.port} id={props.name} onChange={handleSelectChange}>
      {optionList}
    </select>
  );
};

export default function PortsList({ motorPorts }) {

  /* Left and right have default ports */

  const ports = ["2", "3", "4", "5", "6", "7", "8", "9"];

  const [selectedValues, setSelectedValues] = useState(Array(ports.length).fill(() => alert('default')));

  const handleSelectChange = (index, value) => {
    const newSelectedValues = [...selectedValues];
    newSelectedValues[index] = value;
    setSelectedValues(newSelectedValues);
  }

  const handleButtonClick = (event, index) => {
    event.preventDefault();
    console.log(index);
    console.log(selectedValues);
    const func = selectedValues[index];
    func();
  }

  let allListElements = [];

  for (let i = 0; i < ports.length; i++) {
    let element = (
      <div key={i} className="form-group row">
        <label htmlFor={ports[i]} className="col-md-4 d-flex">Port {ports[i]}:</label>
        <button className="btn btn-secondary" onClick={(event) => handleButtonClick(event, i)}>Click me</button>
        <div className="col-md-8">
          <Ports name={i} portName={ports[i]} motorPorts={motorPorts} onChange={(value) => handleSelectChange(i, value)} />
        </div>
      </div>
    );
    allListElements.push(element);

  }

  return (
    <div className="port-form collapse" id="ports-list">
      {allListElements}
    </div>
  );
}
