...
import { commands, X_BTN, MIC_BTN, MIC_BTNON, ACT_MIC_CHATBOT } from '../utils/Constants.js';
...
Chatbot2.propTypes = {
	...
	setActiveMicComponent: PropTypes.func.isRequired,
	activeMicComponent: PropTypes.string.isRequired,
	mic: PropTypes.bool.isRequired,
	setMic: PropTypes.func.isRequired
};

function Chatbot2({ setParentContext, selectedBotName, setActiveMicComponent, activeMicComponent, mic, setMic }) {
	...
	const sendContext = (e) => {
		...
		axios({
			...
			.catch(function (error) {
				...
				else console.log('Chatbot', error);
			});
	};
	...
	const toggleMic = (e) => {
		...
		if (contextMode || selectedBotName) {
			if (activeMicComponent == ACT_MIC_CHATBOT) {
				var temp = !mic;
				setMic(temp);
			} else setActiveMicComponent(ACT_MIC_CHATBOT);
		} else {
	...
	useEffect(() => {
		if (!contextMode) {
			...
			if (queue.length > lastLen) {
				...
				if (Object.prototype.hasOwnProperty.call(commands, queue[queue.length - 2])) {
	...
	return (
		<div
			className={'floating-chat enter ' + expand}
			style={expand === 'expand' ? (right ? styles.leftWindow : styles.rightWindow) : styles.empty}
			onClick={(e) => openChatbox(e)}
		>
			{' '}
			{/* add 'expand' to class for this to turn into a chat */}
			<i className='fa fa-comments' aria-hidden={true}></i>
			<div className={'chat ' + enter}>
				{' '}
				{/* add 'enter' to class for the rest to display */}
				<div className='header'>
					{/* buttons in the header used for changing appearance of the window */}
					<div className='popup'>
						<button id='popupBut' onClick={(e) => toggleChangeFont(e)}>
					...
					<button id='contextLabel' onClick={(e) => toggleMode(e)}>
						{' '}
						{contextMode ? 'Q&A Mode' : 'Command Mode'}
					</button>
					&nbsp;
				...
				<ul className='messages' id='scroll'>
					<div className='date' style={fontSize.body}>
						{date}
					</div>
					...
					{messages.map((item) => (
						<div key={item.id}>
							...
							<li className={'timestamp ' + item.who + 't'} style={fontSize.body}>
								{item.timeStamp}
							</li>
						</div>
					))}
				...
				<div className='footer'>
					...
					<textarea
						...
						placeholder={
							contextMode
								? 'Enter a context/question'
								: selectedBotName != ''
								? 'Click the microphone to send a command'
								: 'Please connect to a Minibot!'
						}
						onKeyPress={(e) => {
							if (contextMode) {
								if (e.key === 'Enter') {
					...
					<div style={{ height: '50px' }}>
						...
						{contextMode ? (
							<span>
								<span>
									<button
										style={{ marginLeft: '2px', objectFit: 'inline', width: '30%' }}
										onClick={(e) => {
											sendContext(e);
										}}
									>
								...
								<span>
									<button
										style={{ marginLeft: '2px', objectFit: 'inline' }}
										onClick={(e) => {
											sendQuestion(e);
										}}
									>
