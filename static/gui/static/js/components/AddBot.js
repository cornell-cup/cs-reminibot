...
function Ports(props) {
  ...
  for (let i = 0; i < ports.length; i++) {
    buttonList.push(
      <li key={i}>
        <button className='btn_ports' onClick={() => props.motorPorts(props.portName, ports[i])}>
          {ports[i]}
        </button>
      </li>
    )
...
function PortsList(props) {
  ...
  const portLabels = ['Left Motor', 'Right Motor', 'Motor 3', 'Line Follower', 'Infrared', 'RFID', 'Ultrasonic']
...
class SpeechRecognition extends React.Component {
  ...
  render() {
    let x = this.state.on ? 'Stop Speech Recognition' : 'Start Speech Recognition'
    return (
      <div>
        <button className='btn btn-danger element-wrapper' onClick={this.toggle}>
          {x}
        </button>
      </div>
    )
...
export default class AddBot extends React.Component {
  ...
  componentDidMount() {
    ...
    document.getElementById('setup_control_tab').addEventListener('keydown', this.handleArrowKeyDown)
  }
  ...
  refreshAvailableBots() {
    ...
    axios({
      ...
      .then(function (response) {
        ...
        if (!(_this.props.selectedBotName === '') && !_this.state.availableBots.includes(_this.props.selectedBotName)) {
  ...
  addBotListener(event) {
    ...
    let botName = this.refreshingBotListRef.current == null ? '' : this.refreshingBotListRef.current.state.currentBot
    ...
    return (
      ...
      <div id='setup_control_tab' tabIndex='-1' className='container-fluid control'>
        <div className='container'>
          ...
          <div className='row'>
            <div className='col horizontalDivCenter'>
              ...
              <Button id='remove-bot' name='Remove' onClick={() => _this.deleteBotListener()} style={_this.props.selectedBotStyle} />
            </div>
          </div>
          <br />
        </div>
        ...
        <div className='container'>
          ...
          <div className='row'>
            <div className='col text-center'>
              <button className='button-movement' onClick={() => this.buttonMapListener('forward')}>
                forward
              </button>
              <br />
              <button className='button-movement' onClick={() => this.buttonMapListener('left')}>
                left
              </button>
              <button className='button-stop' onClick={() => this.buttonMapListener('stop')}>
                stop
              </button>
              <button className='button-movement' onClick={() => this.buttonMapListener('right')}>
                right
              </button>
              <br />
              <button className='button-movement' onClick={() => this.buttonMapListener('backward')}>
                backward
              </button>
              <br />
              <br />
              <form className='horizontalDivCenter'>
                ...
                <input
                  ...
                  max='100'
                  step='5'
                  onChange={(evt) => this.updatePowerValue(evt)}
                />
              </form>
            </div>
          </div>
        ...
        <div className='container'>
          ...
          <div className='col horizontalDivCenter'>
            <SpeechRecognition selectedBotName={this.props.selectedBotName} float='right' />
          </div>
          <div className='col horizontalDivCenter'>
        ...
        <div className='container'>
          <div className='row'>
            <div className='col horizontalDivCenter'>
              ...
              <button className='btn btn-success element-wrapper mr-1' onClick={() => this.setObjectOnClick('object_detection')}>
                Object Detection
              </button>
              <button className='btn btn-primary element-wrapper mr-1' onClick={() => this.setColorOnClick('color_detection')}>
                Color Detection
              </button>
            </div>
          </div>
