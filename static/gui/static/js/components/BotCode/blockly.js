...
class PythonEditor extends React.Component {
  ...
  updateCode(code) {
    ...
    if (code.trim() === '') codeState = -1
    // update the code state to indicate user updates have been made
    // to the Python code,
    else codeState = this.props.pythonCodeState === -1 ? 0 : this.props.pythonCodeState
  ...
  downloadPython(event) {
    ...
    element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(this.props.pythonCode))
    ...
    return (
      <div id='Python' className='col' style={{ paddingLeft: '40px', marginLeft: '10px' }}>
        ...
        <div className='row'>
          ...
          >
            <CodeMirror ref='editor' value={this.props.pythonCode} onChange={(code) => this.updateCode(code)} options={options} />
          </div>
        </div>
        <div className='row'>
          <div className='col'>
            <form style={{ color: 'white' }}>
              <input type='file' id='upload' multiplesize='1' accept='.py' onChange={this.upload} />
            </form>
          </div>
        </div>
        <div className='row'>
        ...
        <div className='row'>
          <div className='col horizontalDiv'>
            ...
            <Button
              id={'DBlock'}
              onClick={() => {
                if (window.confirm('You Are Deleting Custom Block: ' + this.state.functionName)) this.props.dblock(this.state.functionName)
              }}
              name={'Delete'}
            />
            <Button
              id={'DBlockAll'}
              onClick={() => {
                if (window.confirm('You Are Deleting All Custom Blocks')) this.props.dblockAll()
              }}
              name={'Delete All'}
            />
          </div>
        ...
        <div className='row'>
          <div className='col horizontalDiv'>
            <div>
              <textarea id='error-message' placeholder='Script Execution Result' rows='1' cols='60' />
            </div>
            <div className='element-wrapper'>
              <textarea style={{ color: 'blue' }} placeholder='Time Elapsed Coding' id='time' rows='1' cols='20' />
            </div>
...
export default class MinibotBlockly extends React.Component {
  ...
  scriptToCode() {
    ...
    if (pythonCodeState === 0) {
      let msg =
        'The Python Coding Box has user changes.  Can Blockly ' +
    ...
    let code = pythonCodeState < 0 ? window.Blockly.Python.workspaceToCode(this.workspace) : this.props.pythonCode
  ...
  handleLogin(event) {
    ...
    axios({
      ...
      .then((response) => {
        _this.props.redefineCustomBlockList(JSON.parse(response.data.custom_function))
              ...
              <div className='row'>
                <div className='col horizontalDiv'>
                  <form style={{ color: 'white', paddingLeft: '40px' }}>
                    <input type='file' id='blockUpload' multiplesize='1' accept='.xml' onChange={this.loadFileAsBlocks} />
                  </form>
                </div>
              </div>
              <div className='row'>
                <div className='col horizontalDiv'>
                  <div className='element-wrapper' style={{ paddingLeft: '40px' }}>
                    <input type='text' name='blockly_filename' placeholder='FileName.xml' onChange={this.handleInputChange} />
                  </div>
