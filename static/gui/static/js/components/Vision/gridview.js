...
const GridView = (props) => {
	...
	function generateRegularPolygonDeltas(numberOfSides, sideLength) {
		...
		for (let i = 0; i < numberOfSides; i++) {
			deltas.push({
				x: radius * Math.cos(initialAngleOffset + i * individualVertexAngle),
				y: radius * Math.sin(initialAngleOffset + i * individualVertexAngle)
			});
		}
		return deltas;
	}

	function getDeltasFromVerticesXAndY(x, y, vertices) {
		return vertices.map((vertex) => ({ x: vertex['x'] - x, y: vertex['y'] - y }));
	}

	function getPolygonInfoFromVertices(vertices) {
		const center = vertices.reduce(
			(previousValue, currentValue) => ({
				x: previousValue['x'] + currentValue['x'] / vertices.length,
				y: previousValue['y'] + currentValue['y'] / vertices.length
			}),
			{ x: 0, y: 0 }
		);
	...
	const renderObjects = () => {
		...
		for (const detection of detections) {
			switch (detection['type'] ? String(detection['type'].toLowerCase().trim()) : '') {
				case 'minibot':
			...
			case 'image':
				return (
					<image
						...
						width={scaleFactor * width}
						height={scaleFactor * height}
						fill={scaleFactor * (detection['color'] ? detection['color'] : unknownMeasure)}
