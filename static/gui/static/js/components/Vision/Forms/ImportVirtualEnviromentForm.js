...
import { handleVirtualEnviromentForm } from './FormHandlers';
...
const ImportVirtualEnviromentForm = (props) => {
  ...
  function handleFormSubmit(event) {
    ...
    handleVirtualEnviromentForm(virtualEnviroment, virtualEnviromentFile, props.virtualRoomId);
  }
          ...
          <div className="form-group col-md-6">
            ...
            <div class="input-group">
              <div class="custom-file">
                <input type="file" class="custom-file-input" id="virtualEnviromentFile" accept=".json" onChange={(e) => {
                  if (hasValidFile(e.target.files)) {
                    setVirtualEnviromentFile(e.target.files[0]);
                  }
                }
                } />
                <label class="custom-file-label" for="virtualEnviromentFile">{virtualEnviromentFile ? virtualEnviromentFile.name : "Choose Virtual Enviroment File"}</label>
              </div>
            </div>
          </div>
        </div>
        <button type="submit" className="btn btn-success">Import
        </button>
      </form>
    </React.Fragment >
