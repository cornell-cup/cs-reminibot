...
export function handleVirtualEnviromentImportForm(virtualEnviroment, virtualEnviromentFile, virtualRoomId, clearForm) {
  VirtualEnviroment.createEnviromentFromFile(virtualEnviromentFile)
    .then(async (importedVirtualEnviroment) => {
      virtualEnviroment.virtualObjects = importedVirtualEnviroment.virtualObjects.map((virtualObject) => {
        virtualObject['virtual_room_id'] = virtualRoomId
        return virtualObject
      })
      virtualEnviroment.objectMappings = importedVirtualEnviroment.objectMappings.map((objectMapping) => {
