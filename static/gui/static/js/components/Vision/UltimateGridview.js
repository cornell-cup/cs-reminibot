...
const UltimateGridView = (props) => {
			...
			if (objectInfo) {
				...
				Matter.Body.setPosition(
					...
					Matter.Vector.create(
						currentX + dt * currentSpeed * Math.cos(currentOrientation),
						currentY + dt * currentSpeed * Math.sin(currentOrientation)
					)
				);
	...
	const addboundaries = () => {
		const boundaries = [
			Matter.Bodies.rectangle(
				...
				props.world_height / 2,
				boundary_thickness,
				props.world_height + 2 * boundary_thickness,
				{
					isStatic: true
				}
			),
			Matter.Bodies.rectangle(
				props.world_width + boundary_thickness / 2,
				props.world_height / 2,
				boundary_thickness,
				props.world_height + 2 * boundary_thickness,
				{
			...
			Matter.Bodies.rectangle(
				props.world_width / 2,
				-boundary_thickness / 2,
				props.world_width + 2 * boundary_thickness,
				boundary_thickness,
				{
					isStatic: true
				}
			...
			Matter.Bodies.rectangle(
				props.world_width / 2,
				props.world_height + boundary_thickness / 2,
				props.world_width + 2 * boundary_thickness,
				boundary_thickness,
				{
					isStatic: true
	...
	const addDetections = (detectionsToAdd) => {
		for (const detectionToAdd of detectionsToAdd) {
			if (detectionToAdd) {
				...
				switch (detectionClone['type'] ? String(detectionClone['type'].toLowerCase().trim()) : '') {
					case 'minibot':
	...
	const addShape = (detection, imagePath = null) => {
		...
		const vertices = deltas_to_vertices.map((currentValue) => Matter.Vector.create(x + currentValue['x'], y + currentValue['y']));

		//set back to null
		let newObject = null;
		switch (detection['shape'] ? String(detection['shape'].toLowerCase().trim()) : '') {
			case 'cube':
	...
	const renderXAxis = () => {
		...
		for (let i = 0; i < numXAxisTicks; i++) {
			ticks.push(
				<g className='tick' opacity='1' transform={`translate(${scaleFactor * distanceBetweenTicks * i},0)`}>
					<line stroke='currentColor' y2={scaleFactor * props.world_height} strokeWidth='5'></line>
				</g>
			);
		}
		return (
			<g className='x-axis' fill='none' fontSize='40' fontFamily='sans-serif' textAnchor='middle'>
				{ticks}
			</g>
		);
	};
	...
	const renderYAxis = () => {
		...
		for (let i = 0; i < numYAxisTicks; i++) {
			ticks.push(
				<g className='tick' opacity='1' transform={`translate(0,${scaleFactor * distanceBetweenTicks * i})`}>
					<line stroke='currentColor' x2={scaleFactor * props.world_width} strokeWidth='5'></line>
				</g>
			);
		}
		return (
			<g className='y-axis' fill='none' fontSize='40' fontFamily='sans-serif' textAnchor='start'>
				{ticks}
			</g>
		);
	};
	...
	const renderGrid = () => {
		return (
			<React.Fragment>
				<rect width={scaleFactor * props.world_width} height={scaleFactor * props.world_height} fill='white'></rect>
	...
	const renderShapeGroup = (detection, image_path = null) => {
		...
		return (
			<g
				onClick={() => {
					alert(
						`${detection['name'] ? detection['name'] : ''}: (${Math.round(x_pos)},${Math.round(y_pos)}) ${Math.round(
							orientation_pos
						)}°, id: ${detection['id']}`
					);
				}}
			>
				<title>{`${detection['name'] ? detection['name'] : ''}: (${Math.round(x_pos)},${Math.round(y_pos)}) ${Math.round(
					orientation_pos
				)}°, id: ${detection['id']}`}</title>
	...
	const renderShape = (detection, image_path) => {
		...
		const deltas_to_vertices = detection['deltas_to_vertices'] ? detection['deltas_to_vertices'] : [];
		const vertices = deltas_to_vertices.map((currentValue) => new Vector(currentValue['x'], currentValue['y']));
		...
		const average_deltas_to_vertices_radius = deltas_to_vertices.reduce(
			(previousValue, currentValue) =>
				previousValue +
				Math.sqrt(currentValue['x'] * currentValue['x'] + currentValue['y'] * currentValue['y']) / deltas_to_vertices.length,
			0
		);
		switch (detection['shape'] ? String(detection['shape'].toLowerCase().trim()) : '') {
			case 'cube':
	...
	const renderSVG = () => {
		...
		return (
			<svg
				...
				fill='white'
				viewBox={`0 0 ${scaleFactor * props.world_width + 2 * widthPadding} ${scaleFactor * props.world_height + 2 * heightPadding}`}
			>
	...
	return (
		<React.Fragment>
			{!props.defaultEnabled && (
				<button onClick={toggleVisionDisplay} className='btn btn-secondary ml-1'>
			...
			{props.experimentalFeaturesEnabled && (
				<React.Fragment>
					...
					<input
						...
						value={minibotId}
						onChange={(e) => {
							setMinibotId(e.target.value);
						}}
					/>
					<button onClick={runCode} className='btn btn-success ml-1'>
						Run Code
					</button>
					<button
						onClick={() => {
