...
const throwUnequalLengthsError = (v1,v2) => {
  throw new Error(
    `Vector lengths must be equal. This vector's length: ${v1.length}. Other vector's length: ${v2.length}.`
  );
};

const throwVectorTooSmallError = (minimumLength) => {
    throw new Error(
      `Vector(s) length(s) must be at least ${minimumLength} for this function.`
    );
  };
