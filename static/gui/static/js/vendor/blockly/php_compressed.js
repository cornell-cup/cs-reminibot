  ...
  if (!a.outputConnection || !a.outputConnection.targetConnection) {
    ...
    for (var e = 0; e < a.inputList.length; e++)
      a.inputList[e].type == Blockly.INPUT_VALUE &&
        ...
        (d = Blockly.PHP.allNestedComments(d)) &&
        (c += Blockly.PHP.prefixLines(d, '// '))
  }
...
Blockly.PHP.getAdjusted = function (a, b, c, d, e) {
  ...
  a =
    0 < c
      ? Blockly.PHP.valueToCode(a, b, Blockly.PHP.ORDER_ADDITION) || g
      : 0 > c
      ? Blockly.PHP.valueToCode(a, b, Blockly.PHP.ORDER_SUBTRACTION) || g
      : d
      ? Blockly.PHP.valueToCode(a, b, Blockly.PHP.ORDER_UNARY_NEGATION) || g
      : Blockly.PHP.valueToCode(a, b, e) || g
  if (Blockly.isNumber(a)) (a = parseFloat(a) + c), d && (a = -a)
  else {
    if (0 < c) {
      a = a + ' + ' + c
      var f = Blockly.PHP.ORDER_ADDITION
    } else 0 > c && ((a = a + ' - ' + -c), (f = Blockly.PHP.ORDER_SUBTRACTION))
...
Blockly.PHP.lists_create_with = function (a) {
  for (var b = Array(a.itemCount_), c = 0; c < a.itemCount_; c++)
    b[c] = Blockly.PHP.valueToCode(a, 'ADD' + c, Blockly.PHP.ORDER_COMMA) || 'null'
  b = 'array(' + b.join(', ') + ')'
...
Blockly.PHP.lists_isEmpty = function (a) {
  return [
    'empty(' + (Blockly.PHP.valueToCode(a, 'VALUE', Blockly.PHP.ORDER_FUNCTION_CALL) || 'array()') + ')',
...
Blockly.PHP.lists_getIndex = function (a) {
  ...
  switch (a.getFieldValue('WHERE') || 'FROM_START') {
    ...
    case 'FROM_START':
      ...
      if ('GET' == b)
        return (
          (c = Blockly.PHP.valueToCode(a, 'VALUE', Blockly.PHP.ORDER_MEMBER) || 'array()'), [c + '[' + d + ']', Blockly.PHP.ORDER_MEMBER]
        )
      if ('GET_REMOVE' == b)
        return (
          (c = Blockly.PHP.valueToCode(a, 'VALUE', Blockly.PHP.ORDER_COMMA) || 'array()'),
          ['array_splice(' + c + ', ' + d + ', 1)[0]', Blockly.PHP.ORDER_FUNCTION_CALL]
        )
      if ('REMOVE' == b)
        return (c = Blockly.PHP.valueToCode(a, 'VALUE', Blockly.PHP.ORDER_COMMA) || 'array()'), 'array_splice(' + c + ', ' + d + ', 1);\n'
      break
    case 'FROM_END':
      if ('GET' == b)
        return (
          ...
          ['array_slice(' + c + ', ' + d + ', 1)[0]', Blockly.PHP.ORDER_FUNCTION_CALL]
        )
...
Blockly.PHP.lists_setIndex = function (a) {
  ...
  switch (c) {
    case 'FIRST':
      ...
      if ('INSERT' == b)
        return (c = Blockly.PHP.valueToCode(a, 'LIST', Blockly.PHP.ORDER_COMMA) || 'array()'), 'array_unshift(' + c + ', ' + d + ');\n'
      break
    case 'LAST':
    ...
    case 'FROM_START':
      ...
      if ('SET' == b)
        return (c = Blockly.PHP.valueToCode(a, 'LIST', Blockly.PHP.ORDER_MEMBER) || 'array()'), c + '[' + e + '] = ' + d + ';\n'
      if ('INSERT' == b)
        return (
          (c = Blockly.PHP.valueToCode(a, 'LIST', Blockly.PHP.ORDER_COMMA) || 'array()'),
    ...
    case 'RANDOM':
      ...
      c.match(/^\$\w+$/)
        ? (a = '')
        : ((a = Blockly.PHP.variableDB_.getDistinctName('tmp_list', Blockly.Variables.NAME_TYPE)),
          (e = a + ' = &' + c + ';\n'),
          (c = a),
          (a = e))
...
Blockly.PHP.math_single = function (a) {
  ...
  if ('NEG' == b)
    return (
      (a = Blockly.PHP.valueToCode(a, 'NUM', Blockly.PHP.ORDER_UNARY_NEGATION) || '0'),
      '-' == a[0] && (a = ' ' + a),
...
Blockly.PHP.procedures_defreturn = function (a) {
  for (var b = [], c = 0, d; (d = a.workspace.variableList[c]); c++)
    -1 == a.arguments_.indexOf(d) && b.push(Blockly.PHP.variableDB_.getName(d, Blockly.Variables.NAME_TYPE))
  b = b.length ? '  global ' + b.join(', ') + ';\n' : ''
  d = Blockly.PHP.variableDB_.getName(a.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE)
  var e = Blockly.PHP.statementToCode(a, 'STACK')
  Blockly.PHP.STATEMENT_PREFIX &&
    (e = Blockly.PHP.prefixLines(Blockly.PHP.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'"), Blockly.PHP.INDENT) + e)
  ...
  g && (g = '  return ' + g + ';\n')
  for (var f = [], c = 0; c < a.arguments_.length; c++) f[c] = Blockly.PHP.variableDB_.getName(a.arguments_[c], Blockly.Variables.NAME_TYPE)
  b = 'function ' + d + '(' + f.join(', ') + ') {\n' + b + e + g + '}'
...
Blockly.PHP.procedures_callreturn = function (a) {
  for (
    var b = Blockly.PHP.variableDB_.getName(a.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE), c = [], d = 0;
    d < a.arguments_.length;
    d++
  )
    c[d] = Blockly.PHP.valueToCode(a, 'ARG' + d, Blockly.PHP.ORDER_COMMA) || 'null'
...
Blockly.PHP.procedures_ifreturn = function (a) {
  ...
  a.hasReturnValue_
    ? ((a = Blockly.PHP.valueToCode(a, 'VALUE', Blockly.PHP.ORDER_NONE) || 'null'), (b += '  return ' + a + ';\n'))
    : (b += '  return;\n')
  return b + '}\n'
}
Blockly.PHP.texts = {}
...
Blockly.PHP.text_join = function (a) {
  ...
  for (var b = Array(a.itemCount_), c = 0; c < a.itemCount_; c++)
    b[c] = Blockly.PHP.valueToCode(a, 'ADD' + c, Blockly.PHP.ORDER_COMMA) || "''"
...
Blockly.PHP.text_indexOf = function (a) {
  ...
  return [
    Blockly.PHP.provideFunction_('FIRST' == a.getFieldValue('END') ? 'text_indexOf' : 'text_lastIndexOf', [
...
Blockly.PHP.text_changeCase = function (a) {
  ...
  if ('UPPERCASE' == a.getFieldValue('CASE')) var c = 'strtoupper(' + b + ')'
    ...
    'LOWERCASE' == a.getFieldValue('CASE')
      ? (c = 'strtolower(' + b + ')')
      : 'TITLECASE' == a.getFieldValue('CASE') && (c = 'ucwords(strtolower(' + b + '))')
...
Blockly.PHP.controls_repeat_ext = function (a) {
  var b = a.getField('TIMES')
      ...
      : Blockly.PHP.valueToCode(a, 'TIMES', Blockly.PHP.ORDER_ASSIGNMENT) || '0',
  ...
  b.match(/^\w+$/) ||
    Blockly.isNumber(b) ||
    ((e = Blockly.PHP.variableDB_.getDistinctName('repeat_end', Blockly.Variables.NAME_TYPE)), (a += e + ' = ' + b + ';\n'))
...
Blockly.PHP.controls_for = function (a) {
  ...
  if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
    ...
    (a = ''),
      ...
      c.match(/^\w+$/) ||
        Blockly.isNumber(c) ||
        ((f = Blockly.PHP.variableDB_.getDistinctName(b + '_start', Blockly.Variables.NAME_TYPE)), (a += f + ' = ' + c + ';\n')),
      (c = d),
      d.match(/^\w+$/) ||
        Blockly.isNumber(d) ||
        ((c = Blockly.PHP.variableDB_.getDistinctName(b + '_end', Blockly.Variables.NAME_TYPE)), (a += c + ' = ' + d + ';\n')),
...
Blockly.PHP.controls_if = function (a) {
  var b = 0,
    ...
    e
  do
    (e = Blockly.PHP.valueToCode(a, 'IF' + b, Blockly.PHP.ORDER_NONE) || 'false'),
      (d = Blockly.PHP.statementToCode(a, 'DO' + b)),
      (c += (0 < b ? ' else ' : '') + 'if (' + e + ') {\n' + d + '}'),
      ++b
  while (a.getInput('IF' + b))
  a.getInput('ELSE') && ((d = Blockly.PHP.statementToCode(a, 'ELSE')), (c += ' else {\n' + d + '}'))
  return c + '\n'
}
