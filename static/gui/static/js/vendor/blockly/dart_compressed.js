...
Blockly.Dart.init = function (a) {
	...
	a = a.variableList;
	if (a.length) {
		for (var c = 0; c < a.length; c++) b[c] = Blockly.Dart.variableDB_.getName(a[c], Blockly.Variables.NAME_TYPE);
...
Blockly.Dart.lists_create_with = function (a) {
	for (var b = Array(a.itemCount_), c = 0; c < a.itemCount_; c++)
		b[c] = Blockly.Dart.valueToCode(a, 'ADD' + c, Blockly.Dart.ORDER_NONE) || 'null';
...
Blockly.Dart.lists_repeat = function (a) {
	...
	return [
		'new List.filled(' + (Blockly.Dart.valueToCode(a, 'NUM', Blockly.Dart.ORDER_NONE) || '0') + ', ' + b + ')',
...
Blockly.Dart.lists_getIndex = function (a) {
	...
	var c = a.getFieldValue('MODE') || 'GET',
		...
		e =
			Blockly.Dart.valueToCode(a, 'VALUE', 'RANDOM' == d || 'FROM_END' == d ? Blockly.Dart.ORDER_NONE : Blockly.Dart.ORDER_UNARY_POSTFIX) ||
			'[]';
	if ((('RANDOM' != d || 'REMOVE' != c) && 'FROM_END' != d) || e.match(/^\w+$/))
		switch (d) {
			...
			case 'RANDOM':
				...
				if ('REMOVE' == c)
					return (
						(c = Blockly.Dart.variableDB_.getDistinctName('tmp_x', Blockly.Variables.NAME_TYPE)),
		...
		if ('RANDOM' == d)
			return (
				...
				(a = b()),
				(c = Blockly.Dart.variableDB_.getDistinctName('tmp_x', Blockly.Variables.NAME_TYPE)),
				a + ('int ' + c + ' = new Math.Random().nextInt(' + e + '.length);\n') + (e + '.removeAt(' + c + ');\n')
			);
		if ('REMOVE' == c)
			return (
				(d = Blockly.Dart.getAdjusted(a, 'AT', 1, !1, Blockly.Dart.ORDER_ADDITIVE)),
		...
		(e = Blockly.Dart.getAdjusted(a, 'AT1')),
			...
			(a =
				Blockly.Dart.provideFunction_('lists_get_sublist', [
					'List ' + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + '(list, where1, at1, where2, at2) {',
					'  int getAt(where, at) {',
					"    if (where == 'FROM_END') {",
					'      at = list.length - 1 - at;',
					"    } else if (where == 'FIRST') {",
...
Blockly.Dart.lists_sort = function (a) {
	...
	return [
		Blockly.Dart.provideFunction_('lists_sort', [
			'List ' + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + '(list, type, direction) {',
			'  var compareFuncs = {',
...
Blockly.Dart.math_number = function (a) {
	...
	var b;
	Infinity == a
		? ((a = 'double.INFINITY'), (b = Blockly.Dart.ORDER_UNARY_POSTFIX))
		: -Infinity == a
		? ((a = '-double.INFINITY'), (b = Blockly.Dart.ORDER_UNARY_PREFIX))
		: (b = 0 > a ? Blockly.Dart.ORDER_UNARY_PREFIX : Blockly.Dart.ORDER_ATOMIC);
	return [a, b];
};
Blockly.Dart.math_arithmetic = function (a) {
	var b = {
		...
		c = b[0],
		b = b[1],
		d = Blockly.Dart.valueToCode(a, 'A', b) || '0';
...
Blockly.Dart.math_single = function (a) {
	...
	if ('NEG' == b)
		return (
			(a = Blockly.Dart.valueToCode(a, 'NUM', Blockly.Dart.ORDER_UNARY_PREFIX) || '0'),
			'-' == a[0] && (a = ' ' + a),
	...
	a =
		'ABS' == b || 'ROUND' == b.substring(0, 5)
			? Blockly.Dart.valueToCode(a, 'NUM', Blockly.Dart.ORDER_UNARY_POSTFIX) || '0'
			: 'SIN' == b || 'COS' == b || 'TAN' == b
			? Blockly.Dart.valueToCode(a, 'NUM', Blockly.Dart.ORDER_MULTIPLICATIVE) || '0'
...
Blockly.Dart.procedures_defreturn = function (a) {
	...
	Blockly.Dart.STATEMENT_PREFIX &&
		(c = Blockly.Dart.prefixLines(Blockly.Dart.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'"), Blockly.Dart.INDENT) + c);
...
Blockly.Dart.procedures_callreturn = function (a) {
	for (
		var b = Blockly.Dart.variableDB_.getName(a.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE), c = [], d = 0;
		d < a.arguments_.length;
		d++
	)
		c[d] = Blockly.Dart.valueToCode(a, 'ARG' + d, Blockly.Dart.ORDER_NONE) || 'null';
...
Blockly.Dart.procedures_ifreturn = function (a) {
	...
	a.hasReturnValue_
		? ((a = Blockly.Dart.valueToCode(a, 'VALUE', Blockly.Dart.ORDER_NONE) || 'null'), (b += '  return ' + a + ';\n'))
		: (b += '  return;\n');
	return b + '}\n';
};
Blockly.Dart.texts = {};
...
Blockly.Dart.text_join = function (a) {
	switch (a.itemCount_) {
		...
		case 1:
			return [
				(Blockly.Dart.valueToCode(a, 'ADD0', Blockly.Dart.ORDER_UNARY_POSTFIX) || "''") + '.toString()',
				Blockly.Dart.ORDER_UNARY_POSTFIX
			];
		default:
			for (var b = Array(a.itemCount_), c = 0; c < a.itemCount_; c++)
				b[c] = Blockly.Dart.valueToCode(a, 'ADD' + c, Blockly.Dart.ORDER_NONE) || "''";
			a = '[' + b.join(',') + '].join()';
			return [a, Blockly.Dart.ORDER_UNARY_POSTFIX];
	}
};
		...
		(e = Blockly.Dart.getAdjusted(a, 'AT1')),
			...
			(a =
				Blockly.Dart.provideFunction_('text_get_substring', [
					'List ' + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + '(text, where1, at1, where2, at2) {',
					'  int getAt(where, at) {',
					"    if (where == 'FROM_END') {",
					'      at = text.length - 1 - at;',
					"    } else if (where == 'FIRST') {",
...
Blockly.Dart.text_trim = function (a) {
	var b = { LEFT: ".replaceFirst(new RegExp(r'^\\s+'), '')", RIGHT: ".replaceFirst(new RegExp(r'\\s+$'), '')", BOTH: '.trim()' }[
...
Blockly.Dart.text_prompt_ext = function (a) {
	...
	'NUMBER' == a.getFieldValue('TYPE') &&
		((Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;"), (b = 'Math.parseDouble(' + b + ')'));
	return [b, Blockly.Dart.ORDER_UNARY_POSTFIX];
};
...
Blockly.Dart.controls_repeat_ext = function (a) {
	var b = a.getField('TIMES')
			...
			: Blockly.Dart.valueToCode(a, 'TIMES', Blockly.Dart.ORDER_ASSIGNMENT) || '0',
	...
	b.match(/^\w+$/) ||
		Blockly.isNumber(b) ||
		((e = Blockly.Dart.variableDB_.getDistinctName('repeat_end', Blockly.Variables.NAME_TYPE)), (a += 'var ' + e + ' = ' + b + ';\n'));
...
Blockly.Dart.controls_for = function (a) {
	...
	if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
		...
		(a = ''),
			...
			c.match(/^\w+$/) ||
				Blockly.isNumber(c) ||
				((g = Blockly.Dart.variableDB_.getDistinctName(b + '_start', Blockly.Variables.NAME_TYPE)), (a += 'var ' + g + ' = ' + c + ';\n')),
			(c = d),
			d.match(/^\w+$/) ||
				Blockly.isNumber(d) ||
				((c = Blockly.Dart.variableDB_.getDistinctName(b + '_end', Blockly.Variables.NAME_TYPE)), (a += 'var ' + c + ' = ' + d + ';\n')),
			(d = Blockly.Dart.variableDB_.getDistinctName(b + '_inc', Blockly.Variables.NAME_TYPE)),
			(a += 'num ' + d + ' = '),
...
Blockly.Dart.controls_if = function (a) {
	var b = 0,
		...
		e;
	do
		(e = Blockly.Dart.valueToCode(a, 'IF' + b, Blockly.Dart.ORDER_NONE) || 'false'),
			(d = Blockly.Dart.statementToCode(a, 'DO' + b)),
			(c += (0 < b ? 'else ' : '') + 'if (' + e + ') {\n' + d + '}'),
			++b;
	while (a.getInput('IF' + b));
	a.getInput('ELSE') && ((d = Blockly.Dart.statementToCode(a, 'ELSE')), (c += ' else {\n' + d + '}'));
	return c + '\n';
};
