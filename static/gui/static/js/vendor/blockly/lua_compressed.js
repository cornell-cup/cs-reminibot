...
Blockly.Lua.scrub_ = function (a, b) {
  var c = ''
  if (!a.outputConnection || !a.outputConnection.targetConnection) {
    var d = a.getCommentText()
    ;(d = Blockly.utils.wrap(d, Blockly.Lua.COMMENT_WRAP - 3)) && (c += Blockly.Lua.prefixLines(d, '-- ') + '\n')
    for (var e = 0; e < a.inputList.length; e++)
      a.inputList[e].type == Blockly.INPUT_VALUE &&
        (d = a.inputList[e].connection.targetBlock()) &&
        (d = Blockly.Lua.allNestedComments(d)) &&
        (c += Blockly.Lua.prefixLines(d, '-- '))
  }
  e = a.nextConnection && a.nextConnection.targetBlock()
  e = Blockly.Lua.blockToCode(e)
  return c + b + e
}
Blockly.Lua.lists = {}
...
Blockly.Lua.lists_create_with = function (a) {
  for (var b = Array(a.itemCount_), c = 0; c < a.itemCount_; c++)
    b[c] = Blockly.Lua.valueToCode(a, 'ADD' + c, Blockly.Lua.ORDER_NONE) || 'None'
  ...
  return [
    ('GET' == b
      ? Blockly.Lua.provideFunction_('list_get_' + c.toLowerCase(), [
          'function ' + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + '(t' + ('FROM_END' == c || 'FROM_START' == c ? ', at)' : ')'),
          '  return t[' + e('t', c, 'at') + ']',
          'end'
        ])
...
Blockly.Lua.lists_setIndex = function (a) {
  ...
  g =
    'SET' == c
      ? g + (b + '[' + f(b, d, e) + '] = ' + a)
      : g + ('table.insert(' + b + ', ' + (f(b, d, e) + ('LAST' == d ? ' + 1' : '')) + ', ' + a + ')')
  return g + '\n'
}
Blockly.Lua.lists_getSublist = function (a) {
  ...
  var f = Blockly.Lua.lists.getIndex_
  return [
    Blockly.Lua.provideFunction_('list_sublist_' + c.toLowerCase() + '_' + d.toLowerCase(), [
      'function ' +
...
Blockly.Lua.math_arithmetic = function (a) {
  var b = {
    ...
    c = b[0],
    b = b[1],
    d = Blockly.Lua.valueToCode(a, 'A', b) || '0'
...
Blockly.Lua.math_single = function (a) {
  ...
  a =
    'SIN' == b || 'COS' == b || 'TAN' == b
      ? Blockly.Lua.valueToCode(a, 'NUM', Blockly.Lua.ORDER_MULTIPLICATIVE) || '0'
...
Blockly.Lua.colour_picker = function (a) {
  return ["'" + a.getFieldValue('COLOUR') + "'", Blockly.Lua.ORDER_ATOMIC]
}
...
Blockly.Lua.procedures_defreturn = function (a) {
  var b = Blockly.Lua.variableDB_.getName(a.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE),
    c = Blockly.Lua.statementToCode(a, 'STACK')
  Blockly.Lua.STATEMENT_PREFIX &&
    (c = Blockly.Lua.prefixLines(Blockly.Lua.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'"), Blockly.Lua.INDENT) + c)
  Blockly.Lua.INFINITE_LOOP_TRAP && (c = Blockly.Lua.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") + c)
  var d = Blockly.Lua.valueToCode(a, 'RETURN', Blockly.Lua.ORDER_NONE) || ''
  d ? (d = '  return ' + d + '\n') : c || (c = '')
  for (var e = [], f = 0; f < a.arguments_.length; f++) e[f] = Blockly.Lua.variableDB_.getName(a.arguments_[f], Blockly.Variables.NAME_TYPE)
  c = 'function ' + b + '(' + e.join(', ') + ')\n' + c + d + 'end\n'
...
Blockly.Lua.procedures_callreturn = function (a) {
  for (
    var b = Blockly.Lua.variableDB_.getName(a.getFieldValue('NAME'), Blockly.Procedures.NAME_TYPE), c = [], d = 0;
    d < a.arguments_.length;
    d++
  )
    c[d] = Blockly.Lua.valueToCode(a, 'ARG' + d, Blockly.Lua.ORDER_NONE) || 'nil'
...
Blockly.Lua.procedures_ifreturn = function (a) {
  ...
  a.hasReturnValue_
    ? ((a = Blockly.Lua.valueToCode(a, 'VALUE', Blockly.Lua.ORDER_NONE) || 'nil'), (b += '  return ' + a + '\n'))
    : (b += '  return\n')
  return b + 'end\n'
}
Blockly.Lua.texts = {}
...
Blockly.Lua.text_join = function (a) {
  ...
  if (1 == a.itemCount_)
    return ['tostring(' + (Blockly.Lua.valueToCode(a, 'ADD0', Blockly.Lua.ORDER_NONE) || "''") + ')', Blockly.Lua.ORDER_HIGH]
  if (2 == a.itemCount_) {
  ...
  for (var b = [], c = 0; c < a.itemCount_; c++) b[c] = Blockly.Lua.valueToCode(a, 'ADD' + c, Blockly.Lua.ORDER_NONE) || "''"
...
Blockly.Lua.text_getSubstring = function (a) {
  ...
  else throw 'Unhandled option (text_getSubstring)'
  d = a.getFieldValue('WHERE2')
...
Blockly.Lua.text_trim = function (a) {
  var b = { LEFT: '^%s*(,-)', RIGHT: '(.-)%s*$', BOTH: '^%s*(.-)%s*$' }[a.getFieldValue('MODE')]
  return [
    'string.gsub(' + (Blockly.Lua.valueToCode(a, 'TEXT', Blockly.Lua.ORDER_NONE) || "''") + ', "' + b + '", "%1")',
    Blockly.Lua.ORDER_HIGH
  ]
}
...
Blockly.Lua.addContinueLabel = function (a) {
  return -1 < a.indexOf(Blockly.Lua.CONTINUE_STATEMENT) ? a + Blockly.Lua.INDENT + '::continue::\n' : a
}
Blockly.Lua.controls_repeat = function (a) {
  var b = parseInt(a.getFieldValue('TIMES'), 10)
  a = Blockly.Lua.statementToCode(a, 'DO') || ''
  a = Blockly.Lua.addContinueLabel(a)
...
Blockly.Lua.controls_repeat_ext = function (a) {
  var b = Blockly.Lua.valueToCode(a, 'TIMES', Blockly.Lua.ORDER_NONE) || '0',
...
Blockly.Lua.controls_whileUntil = function (a) {
  var b = 'UNTIL' == a.getFieldValue('MODE'),
    ...
    d = Blockly.Lua.addLoopTrap(d, a.id),
    d = Blockly.Lua.addContinueLabel(d)
  b && (c = 'not ' + c)
  return 'while ' + c + ' do\n' + d + 'end\n'
}
Blockly.Lua.controls_for = function (a) {
  var b = Blockly.Lua.variableDB_.getName(a.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE),
    ...
    f = Blockly.Lua.addLoopTrap(f, a.id),
    f = Blockly.Lua.addContinueLabel(f)
  a = ''
  var g
  Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)
    ? ((g = parseFloat(c) <= parseFloat(d)), (e = Math.abs(parseFloat(e))), (g = (g ? '' : '-') + e))
    : ((a = ''),
      (g = Blockly.Lua.variableDB_.getDistinctName(b + '_inc', Blockly.Variables.NAME_TYPE)),
      (a += g + ' = '),
...
Blockly.Lua.controls_if = function (a) {
  var b = 0,
    ...
    e
  do
    (e = Blockly.Lua.valueToCode(a, 'IF' + b, Blockly.Lua.ORDER_NONE) || 'false'),
      (d = Blockly.Lua.statementToCode(a, 'DO' + b)),
      (c += (0 < b ? 'else' : '') + 'if ' + e + ' then\n' + d),
      ++b
  while (a.getInput('IF' + b))
  a.getInput('ELSE') && ((d = Blockly.Lua.statementToCode(a, 'ELSE')), (c += 'else\n' + d))
  return c + 'end\n'
}
